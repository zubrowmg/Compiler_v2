...
void WrapperPass2::getToken(Token tok){
	...
	------------------------------------------*/	
	// Create a Global integer variable if global is specficed, or if we are in the global declare space
	if ((prev2_global && prev_integer && tok.type == "T_IDENTIFIER") || (current_part_of_program == "global_declare" && prev_integer && tok.type == "T_IDENTIFIER")){
		sym.addIdentVariable(tok, "V_Integer", true);
	} 
	// Add am integer variable to the current procedure
	else if ((current_part_of_program == "procedure_declare" && prev_integer && tok.type == "T_IDENTIFIER")){
		sym.addIdentVariable(tok, "V_Integer", false);
	}
	// Adding a new Procedure
	else if (prev_procedure && tok.type == "T_IDENTIFIER"){
		sym.addNewProc(tok);
	} 
